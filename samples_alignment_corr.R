...
# Sort the peaks
peaks <- read.csv("data/MALDI_IHC/results/correlated_masses_reference.csv")$mass
...
write.csv(reference, file = "reference_peaks_corr.csv", row.names = FALSE)
...
for (lame in lames) {
  ...
  writeMSIData(mse_ref, sprintf("%s/%s/results/mse_ref_corr.imzML",
                                config$path_to_data, lame))
  ...
  pData(mse_ref) |>
    as.data.frame() |>
    write_feather(sprintf("%s/%s/results/mse_pixels_corr.feather",
                          config$path_to_data, lame))
  ...
  peaks_ref |>
    ...
    write_feather(sprintf("%s/%s/results/mse_peaks_ref_corr.feather",
                          config$path_to_data, lame))
